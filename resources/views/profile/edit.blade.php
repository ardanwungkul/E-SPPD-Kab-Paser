    ...
    <x-container>
        <x-slot name="content">
            ...
            <form method="post" action="{{ route('profile.update') }}" enctype="multipart/form-data">
                @csrf
                @method('patch')
                <div class=" float-right flex justify-center items-center gap-2 flex-col pl-4 sm:pl-6 lg:pr-20">
                    <div class="relative rounded-lg">
                        <img id="fotoPegawai"
                            class="object-cover w-40 h-40 rounded-full border border-gray-300 cursor-pointer"
                            src="{{ $user->photo ? asset('storage/user/'. $user->photo) : asset('assets/images/placeholder-image.jpg') }}" alt="">
                        <div class="absolute z-10 top-0 rounded-full opacity-0 hover:opacity-100 cursor-pointer">
                            <label for="fotoPegawaiInput" class="rounded-full cursor-pointer">
                                <div class="w-40 h-40 bg-black opacity-60 flex items-center justify-center rounded-full">
                    ...
                    <div class="w-full max-w-40">
                        ...
                        <label for="fotoPegawaiInput"
                            class="block bg-[#249D06] text-white w-full text-center rounded-lg py-1 px-2 text-sm cursor-pointer">Pilih
                            File</label>
