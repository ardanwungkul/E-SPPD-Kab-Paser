<x-app-layout>
    ...
    <x-container>
        <x-slot name="content">
            <form action="{{ route('sub-kegiatan.store') }}" method="POST">
                @csrf
                @method('POST')
                <div class="text-xs md:text-sm space-y-3 max-w-xl mx-auto">
                    ...
                    <div class="flex flex-col gap-1">
                        <label for="kode">Kode Sub. Kegiatam</label>
                        <input type="text" id="kode" name="kode"
                            class="text-xs md:text-sm rounded-lg border border-gray-300 shadow-md"
                            value="{{ old('kode') }}" placeholder="Masukkan Kode Sub. Kegiatan" required>
                    </div>
                    <div class="flex flex-col gap-1">
                        <label for="uraian">Nama Sub. Kegiatan</label>
                        <input type="text" id="uraian" name="uraian"
                            class="text-xs md:text-sm rounded-lg border border-gray-300 shadow-md"
                            value="{{ old('uraian') }}" placeholder="Masukkan Nama Sub. Kegiatan " required>
                    </div>
